...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  if (method === "GET") {
    ...
    if (isOrderApprequest) {
      ...
      //find DisabledMenuCategoryLocation frim menuCategoryId

      const disabledMenuCategoryId =
        (await prisma.disabledMenuCategoryLocation.findMany({
          where: { menuCategoryId: { in: menuCategoryIds } },
        })).map(item=> item.menuCategoryId);
      menuCategories = menuCategories.filter(item=> !disabledMenuCategoryId.includes(item.id))
      //find Menus
      ...
      const disabledMenuIds = (await prisma.disabledMenuLocation.findMany({
        where: { id: { in: menuIds }, isArchived: false },
    ...
    if (!dbUser) {
      ...
      const newCompanyAddress =
        "62 street,132 and 133 bet, Pyi Gyi Tagon Township";
      ...
      const disabledMenuCategoryLocation =
        await prisma.disabledMenuCategoryLocation.findMany({
          where: { menuCategoryId: { in: menuCategoryIds } },
        });
  ...
  res.status(404).send("Menthod is not allowed.");
};
