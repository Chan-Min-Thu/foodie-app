...
import { deleteQrCodeImage } from "@/utlis/fileUpload";
...
export default async function handler(
  ...
  if (method === "POST") {
    ...
    const isValid = name && price !== undefined && menuCategoryId.length;
    ...
    const { id, name, price, menuCategoryId, isAvaliable, locationId, imgUrl } =
      req.body;
    ...
    // if(exit.imgUrl && exit.imgUrl !== imgUrl){
    //   const assetUrl = exit.imgUrl
    //   await deleteQrCodeImage(assetUrl)
    // }
    //get menuCategoryMenuArray[]<--  by changing object<--{menuId,menuCategoryId}<-- getting frontend menuCategoryId<--[] data
    const menuCategoryMenuArray = menuCategoryId.map((categoryId: number) => ({
      menuId: menu.id,
    ...
    if (locationId && isAvaliable === false) {
      const exit = await prisma.disabledMenuLocation.findFirst({
        where: { menuId: menu.id },
      });
      if (exit)
        return res
          .status(200)
          .json({ disabledMenuLocation: exit, menuCategoryMenu, menu });
      const disabledMenuLocation = await prisma.disabledMenuLocation.create({
        data: { menuId: menu.id, locationId },
      });
      return res
        .status(200)
        .json({ disabledMenuLocation, menuCategoryMenu, menu });
    } else if (locationId && isAvaliable === true) {
      const exit = await prisma.disabledMenuLocation.findFirst({
        where: { menuId: menu.id },
      });
      if (!exit) return res.status(200).json({ menuCategoryMenu, menu });
      const disabledMenuLocation = await prisma.disabledMenuLocation.deleteMany(
        { where: { menuId: exit.menuId, locationId } }
      );
      return res
        .status(200)
        .json({ disabledMenuLocation, menuCategoryMenu, menu });
    }
    const disabledMenuLocation = await prisma.disabledMenuLocation.findMany({
      where: { menuId: id },
    });
    return res
      .status(200)
      .json({ disabledMenuLocation, menu, menuCategoryMenu });
    ...
    await prisma.addOnCategoryMenu.updateMany({
      data: { isArchived: true },
      where: { menuId: menu.id },
    });
